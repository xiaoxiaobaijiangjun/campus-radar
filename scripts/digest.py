        ...
        for r in recs:
            ...
            if deadline:
                try:
                    ...
                    if days_left < 0:
                        lines.append(f"  ⚠️ 截止日期：{deadline}（已过期）")
                    elif days_left <= 7:
                        lines.append(f"  🔴 截止日期：{deadline}（仅剩 {days_left} 天！）")
                    elif days_left <= 14:
                        lines.append(f"  🟡 截止日期：{deadline}（{days_left} 天后截止）")
                    else:
                        lines.append(f"  截止日期：{deadline}（{days_left} 天后截止）")
                except ValueError:
            ...
            if r.get("apply_url"):
                lines.append(f"  投递入口：{r['apply_url']}")
            lines.append(f"  来源：{r.get('source_platform', '')} · 发现于 {r.get('first_seen', today)}")
        lines.append("")
    # Footer disclaimer
    unverified = sum(1 for r in new_postings if not r.get("verified"))
    if unverified > 0:
        lines.append("---")
        lines.append(f"⚠️ 以上 {unverified} 个岗位的链接来自 AI 搜索，未经逐一核实，可能存在失效情况。")
    return "\n".join(lines).rstrip()
