...
candidates.json: JSON array of objects with fields:
  company, title, city, highlight, source_url, apply_url, deadline,
  source_platform (discovered_date optional, defaults to today)
...
def main():
    ...
    for c in candidates:
        ...
        if h in postings:
            ...
            # Refresh fields that are empty in state but present in candidate
            for field in ("apply_url", "deadline", "highlight", "city", "source_platform"):
                if not postings[h].get(field) and c.get(field):
                    postings[h][field] = c[field]
            continue
        ...
        if fkey in fuzzy_index:
            ...
            for field in ("apply_url", "deadline", "highlight", "city", "source_platform"):
                if not postings[existing_h].get(field) and c.get(field):
                    postings[existing_h][field] = c[field]
            continue
    ...
    state["consecutive_zero_days"] = 0 if new_only else state.get("consecutive_zero_days", 0) + 1
